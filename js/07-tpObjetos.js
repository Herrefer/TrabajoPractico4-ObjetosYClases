...
const contactosListados = [contactoUno, contactoDos, contactoTres, contactoCuatro]
let opcion = parseInt(
...
if (opcion > 7) {
  ...
  switch (opcion) {
    case 1:
      ...
      const contactoAniadir = contactosListados.find((contacto) => contacto === aniadirContacto_)
      if (agenda.listaDeContactos.length <= agenda.tamanioAgenda) {
        agenda.listaDeContactos.push(contactoAniadir);
        console.log(`${contactoAniadir} fué añadido correctamente a la agenda`);
      } else {
        console.log(
          `${contacto.nombre} no pudo ser añadido, la agenda está llena`
        );
